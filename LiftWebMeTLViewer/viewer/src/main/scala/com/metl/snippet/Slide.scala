...
class SlideSnippet {
	...
	/*def withoutHeaders(in:NodeSeq):NodeSeq = {
	...
	def title(in:NodeSeq):NodeSeq = {
		...
		conversationId.is match {
			case s:String if (s.length > 0) => {
				println("using server: %s".format(server))
				ConversationCache.getConversation(conversationId.is.toInt,server.serverConfig) match {
					case d:Conversation if (d != Conversation.empty) => {
						val prefix = d.slides.find(s => s.id.toString == slideId.is) match {
							case Some(slide) => "Slide %d: ".format(slide.index+1)
							case _ => ""
						}
						Text("%s%s".format(prefix,d.title))
					}
	...
	private def getNearbySlideId(offset:Int):Box[String] = (conversationId.is,slideId.is) match {
		case (cid,sid) if (cid.length>0 && sid.length>0) => {
			HistoryCache.getHistory(sid,server.is.serverConfig) match {
				case h:History if (h != History.empty) => {
					val slides = ConversationCache.getConversation(cid.toInt,server.is.serverConfig).slides
					slides.length match {
						case 0 => Empty
						case 1 => Full(sid)

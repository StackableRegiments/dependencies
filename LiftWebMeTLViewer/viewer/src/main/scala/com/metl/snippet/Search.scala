...
class SearchSnippet {
	...
	def searchResults ={
		query.is match {
			case q:String if (q.length > 0) => {
				val results = server.serverConfig.searchForConversation(q)
				...
				results.length match {
					...
					case _ => {
						...
						"#searchResultsMetaNavigation *" #> {
							...
							<a href={"/?server=%s&q=%s&page=%d".format(server.is.name,Helpers.urlEncode(query.is),Math.max(page.is-1,0))}>‹ Next</a>
						...
						"#searchResults *" #> pageResults.foldLeft(NodeSeq.Empty)((acc,item) => acc ++ renderResult(server.serverConfig,item).apply(SearchTemplates.searchResult))
					}
				} }
			}
	...
	private def renderQuizLinks(server:ServerConfiguration,conversationId:String):NodeSeq =
		HistoryCache.getHistory(conversationId,server).getQuizzes match {

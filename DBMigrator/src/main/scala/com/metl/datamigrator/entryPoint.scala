...
object Main extends App with Logger {
  ...
  servers.filterNot(_ == EmptyBackendAdaptor).foreach(config => {
    ...
    val exportSerializer = new MigratorXmlSerializer(config.name, timezoneOverrides, upperDateTime, lowerDateTime)
    ...
    def exportConversation(onBehalfOfUser: String, conversation: String): Box[NodeSeq] = {
      tryo((for (
        ...
        xml = {
          <export>
            {exportSerializer.fromConversation(conv)}<histories>
            {histories.map(h => exportSerializer.fromHistory(h))}
          </histories>
          </export>
        }
      ) yield {
    ...
    val rawConvs = config.searchForConversation("")
    //.take(25) // hopefully every conversation will be returned by this query?  Will probably have to write an explicit "getAllConversations" call into the backends.

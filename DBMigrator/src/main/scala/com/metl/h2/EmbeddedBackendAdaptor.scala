...
class LocalH2BackendAdaptor(name:String,filename:Option[String]) extends PersistedAdaptor(name,"localhost"){
  override val dbInterface = new H2Interface(this,filename)
}
object LocalH2ServerConfigurator extends ServerConfigurator{
  override def matchFunction(e:Node): Boolean = (e \\ "type").headOption.exists(_.text == "localH2")
  override def interpret(e:Node,messageBusCredentialsFunc:()=>Tuple2[String,String],conversationListenerCredentialsFunc:()=>Tuple2[String,String],httpCredentialsFunc:()=>Tuple2[String,String]):Option[ServerConfiguration] = {
    Some(new LocalH2BackendAdaptor((e \ "name").headOption.map(_.text).getOrElse("localH2"),(e \ "filename").headOption.map(_.text)))
  }
}

class SqlBackendAdaptor(name:String,vendor:StandardDBVendor,startingPoolSize:Int = 0,maxPoolSize:Int = 0) extends PersistedAdaptor(name,"localhost"){
  override val dbInterface = new SqlInterface(this,vendor,startingPoolSize,maxPoolSize)
}
object SqlServerConfigurator extends ServerConfigurator{
  ...
  override def interpret(e:Node,messageBusCredentialsFunc:()=>Tuple2[String,String],conversationListenerCredentialsFunc:()=>Tuple2[String,String],httpCredentialsFunc:()=>Tuple2[String,String]) = {
    for (
      ...
      new SqlBackendAdaptor((e \\ "name").headOption.map(_.text).getOrElse("sql"),vendor,startingPool,vendorMaxPoolSize)
    }
  }
}

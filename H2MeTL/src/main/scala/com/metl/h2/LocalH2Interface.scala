...
import _root_.net.liftweb.mapper.{DB, DefaultConnectionIdentifier, Schemifier, StandardDBVendor, _}
...
class SqlInterface(config:ServerConfiguration,vendor:StandardDBVendor,onConversationDetailsUpdated:Conversation=>Unit,startingPool:Int = 0,maxPoolSize:Int = 0,h2ThreadPoolMultiplier:Int = 8) extends PersistenceInterface(config) with Logger{
  ...
  protected val identityPoolTaskSupport = new scala.collection.parallel.ForkJoinTaskSupport(new scala.concurrent.forkjoin.ForkJoinPool(5 * h2ThreadPoolMultiplier))
  protected val stanzaTaskSupport = new scala.collection.parallel.ForkJoinTaskSupport(new scala.concurrent.forkjoin.ForkJoinPool(15 * h2ThreadPoolMultiplier))
